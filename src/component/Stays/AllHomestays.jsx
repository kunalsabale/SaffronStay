...
const AllHomestays = () => {
    ...
    const indexOfLastHomeStays = page * HomeStaysPerPage;
    const indexOfFirstHomeStays = indexOfLastHomeStays - HomeStaysPerPage;
    const currentHomeStays = filteredHomestays.slice(indexOfFirstHomeStays, indexOfLastHomeStays);


    useEffect(() => {
        if (currentHomeStays.length === 0 || !currentHomeStays[0]?.about?.images) return;
        const interval = setInterval(() => {
            setCurrentImageIndex((prevIndex) =>
                (prevIndex + 1) % currentHomeStays[0].about.images.length
            );
        }, 3000);
                        ...
                        : (currentHomeStays.map((val, i) => (
                            ...
                            >
                                <Box sx={{ width: "100%", height: { xs: "200px", sm: "250px", md: "300px" }, borderRadius: "20px", objectFit: "cover", position: "relative", }}>
                                    ...
                                    />
                                    <Typography variant="body2" color="black" sx={{ position: "absolute", borderRadius: "10px", width: "100%", display: "flex", justifyContent: "space-between", p: 1 }}>
